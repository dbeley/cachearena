/**
 * Popup script for CacheArena extension
 * Manages the extension popup UI and user interactions
 */

import type { Cache, ExportResult, ClearResult } from "./types";
import { sendMessage } from "./shared/browser-compat";

const phoneCountEl = document.getElementById("phone-count") as HTMLElement;
...
async function loadStats(): Promise<void> {
  try {
    const cache = await sendMessage<Cache>({ type: "gsmarena-cache-request" });
...
async function handleExport(): Promise<void> {
  ...
  try {
    const result = await sendMessage<ExportResult>({ type: "gsmarena-cache-export" });
...
async function handleClear(): Promise<void> {
  ...
  try {
    await sendMessage<ClearResult>({ type: "gsmarena-cache-clear" });
